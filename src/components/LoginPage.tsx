/**
 * LoginPage — Componente de Autenticação
 * 
 * Componente isolado para a tela de login/registro.
 * Consome o contexto global via useApp().
 */
...
export function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 border border-slate-100">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-black tracking-tight text-slate-900">
            Veiculo<span className="text-emerald-600">Pro</span>
          </h1>
          <p className="text-slate-400 text-sm mt-1">
            {authMode === 'login' ? 'Bem-vindo de volta, motorista!' : 'Crie sua conta gratuita'}
          </p>
        </div>
        ...
        <form onSubmit={authMode === 'login' ? handleLogin : handleRegister} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              <input
                ...
                className="w-full px-4 py-3 pl-11 rounded-xl border border-slate-200 bg-slate-50 text-slate-900 outline-none focus:ring-2 focus:ring-emerald-500 transition-all placeholder:text-slate-400"
                placeholder="••••••••"
                required
              />
          ...
          <button
            type="submit"
            className="w-full bg-emerald-600 text-white font-bold py-4 rounded-xl shadow-lg shadow-emerald-200 hover:bg-emerald-700 transition-all mt-4"
          >
      ...
      <AnimatePresence>
        {toast && <Toast message={toast.message} type={toast.type as any} onClose={clearToast} />}
      </AnimatePresence>
    </div>
  );
}
